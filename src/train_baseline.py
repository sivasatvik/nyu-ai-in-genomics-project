...
from model import BaselineRNNGenerator, EnvDataset, VOCAB_SIZE, CHAR_TO_IDX, IDX_TO_CHAR
import torch
...
def generate_protein(model, scaler, temp_c, precip, radiation, device, max_len=100):
    ...
    with torch.no_grad():
        ...
        for _ in range(max_len):
            ...
            out, hidden_states = model.rnn(embedded, hidden_states)
            prediction = model.fc(out)
...
def main(csv_path, scaler_path, checkpoint_path):
    ...
    model = BaselineRNNGenerator(INPUT_DIM, EMBED_SIZE, HIDDEN_SIZE, VOCAB_SIZE).to(device)
    ...
    def inference_multiple_envs():
        ...
        best_model = BaselineRNNGenerator(INPUT_DIM, EMBED_SIZE, HIDDEN_SIZE, VOCAB_SIZE).to(device)
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "-s",
        "--scaler",
        default="checkpoints/environmental_baseline_scaler_10K.pkl",
        help="Path to scaler pickle file (default: checkpoints/environmental_baseline_scaler_10K.pkl)",
    )
    parser.add_argument(
        "-c",
        "--checkpoint",
